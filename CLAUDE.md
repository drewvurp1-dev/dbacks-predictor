...
**Pooled fleets (tail-pool tracking).** Some teams fly a rotating charter pool with no dedicated airframe — notably ARI on Delta's NBA-owned VIP 757 pool (~11 airframes). AeroDataBox frequently doesn't index the pool's charter callsign (DL8876/DAL8876) under its flight-number endpoint at all, so callsign-only tracking silently returns "no flight found." These teams are instead tracked by the **full pool of tail registrations** (`/flights/reg` is reliable and tails don't shift annually the way callsign blocks do). To keep this from multiplying AeroDataBox quota by the pool size, `lookupTeam` (a) **early-exits** the tail sweep the moment a flight into the destination airport is found — only one airframe flies any given trip — and (b) caches the **resolved airframe** per `ABBR|DEST` (12h TTL) so subsequent polls query that one tail first. Steady-state polling therefore costs ~one tail's worth of calls, not the whole pool's. The callsign is kept as a secondary hint behind the tail sweep.

**Multi-airport destinations.** Some MLB cities use more than one charter airport — notably St. Louis, where charters frequently land at **CPS** (St. Louis Downtown / Cahokia) rather than **STL** (Lambert). `DEST_ALIASES` in `routes/flights.js` maps a primary airport to its accepted alternates, and `lookupTeam` returns a server-computed `arrival.intoDest` boolean (honoring aliases) so the dashboard and cron don't each re-derive airport matching.

**ADS-B fallback (OpenSky).** AeroDataBox is schedule-centric, so a charter into a GA airport like CPS often isn't indexed *by registration* there at all (even when ADS-B trackers show the full track). When AeroDataBox yields no into-dest arrival, `lookupTeam` falls back to **OpenSky's historical flights-by-aircraft** endpoint, keyed by the airframe's ICAO 24-bit hex. US N-numbers are converted to hex mathematically (`nToIcao24`, validated against N652DL→A8947E and N664DN→A8C36D — see `routes/flights.test.js`); OpenSky reports airports in ICAO form, matched against `K`+IATA (Toronto = CYYZ). The fallback only fires on an AeroDataBox dest-miss, queries the resolved/likely airframe first, and early-exits — so it adds no cost on the common path. Disable with `DISABLE_ADSB_FALLBACK=1`; raise the rate limit with `OPENSKY_USER`/`OPENSKY_PASS`.

Flight lookups go through `/flights/team/:abbr` → AeroDataBox via RapidAPI (with the OpenSky ADS-B fallback above). The cron poller (`checkCharterPoll`) pre-warms the in-memory cache in two phases: (1) a scout phase from T-60h before the series opener's first pitch, which fetches the scheduled departure time (ETD) from AeroDataBox and populates the dashboard with "SCHEDULED" state before wheels-up; (2) an active polling phase every 30 min starting at the ETD (falls back to T-6h from opener first pitch if AeroDataBox has no schedule yet), running until the charter lands or ETD+8h (hard cap to bound API quota when AeroDataBox loses the flight). Flight-date lookups span UTC today−2 through tomorrow so pre-departure schedules filed for the next local day are visible to the ETD scout. Opponent resolution in cron.js goes through `TEAM_ID_TO_ABBR` — the schedule API's default response has no `team.abbreviation`. Uses `fetchNextSeriesGame` (2-day lookahead) instead of `fetchTodayGame` so it works on off days and when today's game is already Final.
