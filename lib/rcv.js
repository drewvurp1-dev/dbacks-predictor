...
function instantRunoff(ballots, candidates) {
  ...
  for (let guard = 0; guard <= candidates.length + 1; guard++) {
    ...
    if (batch.length > 1) {
      ...
      round.eliminated = batch.map(id => ({ id, name: names.get(id), votes: counts.get(id) }));
      continue;
    }
    ...
    const loser = _pickLoser(tiedLast, bordaPoints, names);
    remaining.delete(loser);
    round.eliminated = [{ id: loser, name: names.get(loser), votes: counts.get(loser) }];
  }
