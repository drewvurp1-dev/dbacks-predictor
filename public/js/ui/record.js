  ...
  document.getElementById('corbet-bets').innerHTML=visible.map(pg=>{
    const cards=pg.bets.map(b=>{
      ...
      return`<div class="bet-card" style="${cardBg};border-radius:10px;padding:14px 16px;margin-bottom:10px;border:1px solid;">
        ...
        ${b.eliteOverGuard&&!b.conflict?`<div style="background:#180a1a;border:1px solid #401a4a;border-radius:6px;padding:6px 10px;margin:6px 0 8px;font-size:9px;color:#c870e0;font-family:\'Chakra Petch\',monospace;letter-spacing:1px;" data-tip="Listed pitcher's SIERA/xFIP/FIP is ≤3.50 — top-tier ace territory. OVER picks against this caliber of arm are downgraded one notch by default; the structural pitcher hooks can't fully discount Cy Young-level seasons.">⚠ ACE ON THE MOUND — OVER pick downgraded vs elite pitcher.</div>`:''}
        ${b.edgeStrength!=='none'
